...
(function (window) {
  ...
  Seats.prototype._calcSeat = function () {
    ...
    this._options.seatsData.forEach((item) => {
      this._options._maxMap.seatOptions.yMax < item.y &&
        (this._options._maxMap.seatOptions.yMax = item.y);
      this._options._maxMap.seatOptions.yMin > item.y &&
        (this._options._maxMap.seatOptions.yMin = item.y);
      this._options._maxMap.seatOptions.xMax < item.x &&
        (this._options._maxMap.seatOptions.xMax = item.x);
      this._options._maxMap.seatOptions.xMin > item.x &&
        (this._options._maxMap.seatOptions.xMin = item.x);
      if (item.is_best) {
    ...
    this._options._maxMap.seatOptions.xMin > 1 &&
      (this._options.diff_x = this._options._maxMap.seatOptions.xMin - 1);
    this._options._maxMap.seatOptions.yMin > 1 &&
      (this._options.diff_y = this._options._maxMap.seatOptions.yMin - 1);
    ...
    this._options._maxMap.seatOptions.seatSingleW =
      this._options._maxMap.seatOptions.maxW;
    ...
    this._options._maxMap.seatOptions.seatSingleH =
      this._options._maxMap.seatOptions.seatSingleW;
    this._options._maxMap.seatOptions.seatOverAllW =
      (this._options._maxMap.seatOptions.xMax - this._options.diff_x) *
      this._options._maxMap.seatOptions.seatSingleW;
    this._options._maxMap.seatOptions.seatOverAllH =
      (this._options._maxMap.seatOptions.yMax - this._options.diff_y) *
      this._options._maxMap.seatOptions.seatSingleH;
  ...
  Seats.prototype._calcSeatStartCenter = function () {
    ...
    // 座位map宽度
    let seatTR =
      this._options._maxMap.seatOptions.seatOverAllW * zoom +
      seatMidAllW +
      left;
    ...
    if (this._options._maxMap.center.x !== this._options._maxMap.seatCenter.x) {
      ...
      if (num > 0) {
        this._options._maxMap._offset.x =
          (this._options._maxMap.center.x - num + left + (mid * zoom) / 2) / 2;
        this._options._maxMap._offset.x += -mid * zoom;
      } else {
        this._options._maxMap._offset.x =
          (this._options._maxMap.center.x + Math.abs(num) + left) / 2;
        if (
          this._options._maxMap._zoom._current ===
            this._options._maxMap._zoom._max &&
          this._options._maxMap._zoom._max === 1
        ) {
  ...
  Seats.prototype._drawLeftSequence = function () {
    ...
    let yMax = num,
      h =
        ...
        this._options._maxMap._zoom._current,
      mid =
        this._options._maxMap._offset._mid *
        this._options._maxMap._zoom._current;
    while (num--) {
  ...
  Seats.prototype._drawLargeMapCenterLine = function () {
    ...
    this.options.maxMap.ctx.moveTo(center.x - x, offset_y - 10);
  ...
  Seats.prototype.setFooterLogo = function (data) {
    if (!data) {
      throw new Error(
        'setFooterLogo',
        'Seat background image data parameter is empty.',
      );
    }
  ...
  Seats.prototype._createImageEl = function (url) {
    let otherEl = document.querySelector('.other-div-images'),
      isExist = true;
    if (!otherEl) {
  ...
  Seats.prototype._drawBestLine = function () {
    let ctx = this.options.maxMap.ctx,
      ...
      x_all_w =
        (range.xMin - _diff_x - 1 > 0 ? range.xMin - _diff_x - 1 : 0) *
        w *
        zoom,
      x_mid_w =
        (range.xMin - _diff_x - 1 > 0 ? range.xMin - _diff_x - 1 : 0) *
        ...
        zoom,
      y_all_h =
        (range.yMin - _diff_y - 1 > 0 ? range.yMin - _diff_y - 1 : 0) *
        h *
        zoom,
      y_mid_h =
        (range.yMin - _diff_y - 1 > 0 ? range.yMin - _diff_y - 1 : 0) *
  ...
  Seats.prototype._drawLargeMapSeat = function () {
    let ctx = this.options.maxMap.ctx;
    this._options.seatsData.forEach((item, i) => {
  ...
  Seats.prototype._randomGetSold = function () {
    ...
    if (!sold) {
      throw new Error(
        'drawLargeMap',
        'Seat map setting error, sold icon is empty.',
      );
    }
  ...
  Seats.prototype._drawLargeMapRoutineSeat = function (ctx, data) {
    ...
    ctx.fillRect(
      (data.x - diff_x - 1) * this._options._maxMap.seatOptions.seatSingleW +
        (data.x - diff_x) * mid,
      (data.y - diff_y - 1) *
        (this._options._maxMap.seatOptions.seatSingleH + mid),
  ...
  Seats.prototype._clearMaxMap = function () {
    this.options.maxMap.ctx.clearRect(
      0,
      0,
      this.canvasEl.width,
  ...
  Seats.prototype._init = function () {
    ...
    if (
      this.options.minMap &&
      this.options.minMap.el &&
      !this.options.minMap.ctx
    ) {
  ...
  Seats.prototype._canvasClick = function (data) {
    ...
    let seats = this._options.seatsData.filter((item) => {
      let x =
          (item.x - _diff_x - 1 > 0 ? item.x - _diff_x - 1 : 0) * w * zoom +
          left +
          mid * zoom * (item.x - _diff_x),
        y =
          (item.y - _diff_y - 1 > 0 ? item.y - _diff_y - 1 : 0) * h * zoom +
          top +
          (item.y - _diff_y - 1 > 0 ? mid * zoom * (item.y - _diff_y - 1) : 0);
      let _x = x + w * zoom,
        _y = y + h * zoom;
   ...
   * @returns {boolean}
   */
  Seats.prototype.checkRowSeatContinusAndEmpty = function (
    rowNum,
    startPos,
    endPos,
  ) {
    ...
    for (let i = startPos; i <= endPos; i++) {
      for (let j = 0; j < data.length; j++) {
        if (
          data[j].x - this._options.diff_x === i &&
          data[j].y - this._options.diff_y === rowNum &&
          data[j].status !== 0
        ) {
          return false;
        }
  ...
  Seats.prototype.chooseSeat = function (result) {
    ...
    this._options.seatsData.filter((item) => {
      for (let i = 0; i < result.length; i++) {
        if (
          item.x - this._options.diff_x === result[i][1] &&
          item.y - this._options.diff_y === result[i][0]
        ) {
          item.status = 1;
          data.push(item);
  ...
  Seats.prototype._distance = function (row, startPos, endPos, num) {
    ...
    for (let i = startPos; i <= endPos; i++) {
      if (
        i >= best_data.xMin - this._options.diff_x &&
        i <= best_data.xMax - this._options.diff_x
      ) {
        num--;
      }
    }
  ...
  Seats.prototype._updateScreen = function () {
    let zoom = this._options._maxMap._zoom._current,
      ...
      mid = this._options._maxMap._offset._mid,
      mid_w =
        (this._options._maxMap.seatOptions.xMax - this._options.diff_x) * mid;
    let map_w = (all_w + mid_w) * zoom;
    let t =
      (this._options._maxMap.screenW - map_w) / 2 -
      this._options._maxMap._offset.x +
      2 * zoom;
    this.options.maxMap.screen.el.style.transform = 'translateX('.concat(
      -Math.round(t) + mid,
      'px)',
    );
  };
  ...
  Seats.prototype._largePinchMove = function (data) {
    ...
    scale >= this._options._maxMap._zoom._max &&
      (scale = this._options._maxMap._zoom._max);
    ...
    if (
      this._options._maxMap._offset.x - (r_x - scale_x) !==
        this._options._maxMap._offset.x ||
      this._options._maxMap._offset.y - (r_y - scale_y) !==
        this._options._maxMap._offset.y
    ) {
      let x = this._options._maxMap._offset.x - (r_x - scale_x),
  ...
  Seats.prototype._boundary = function () {
    ...
    if (
      ...
      offset_y < this.canvasEl.height - total_h &&
        (offset_y = this.canvasEl.height - total_h);
    }
    if (this.canvasEl.width >= total_w) {
      ...
      offset_x < this.canvasEl.width - total_w &&
        (offset_x = this.canvasEl.width - total_w - left - right);
    }
  ...
  Seats.prototype._setMapCenter = function () {
    ...
    let mapW =
      this._options._maxMap.seatOptions.seatOverAllW +
      mid * (this._options._maxMap.seatOptions.xMax - _diff_x);
    let scaleW = (mapW * zoom) / 2;
    this._options._maxMap.center.x =
      scaleW +
      this._options._maxMap._offset.x +
      (mid * zoom * (is_drag ? zoom : 1)) / 2;
  };
  ...
  Seats.prototype.showAlert = function (color = '') {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        this.options.alert_option.els.div.style.backgroundColor =
          this.options.alert_option.bg_color;
      }, 200);
    }, 3000);
  };
